...
export const SelectTraderAccounts: FC = () => {
  ...
  const fetchTraderAccounts = useCallback(async () => {
    ...
    try {
      ...
      const markeProductGroup = new PublicKey(mpgPubkey);
      const trgs = await manifest.getTRGsOfOwner(owner, markeProductGroup);
      setTrgsArr(trgs);
    } catch (error: any) {
      notify({
        type: "error",
  ...
  const handleCreateTRG = useCallback(async () => {
    try {
      ...
      const markeProductGroup = new PublicKey(mpgPubkey);
      await manifest.createTrg(markeProductGroup);
  ...
  const handleSelection = useCallback(
    async (selectedTrgPubkey: string) => {
      ...
      await trader.update();
      const markeProductGroup = new PublicKey(mpgPubkey);
      await manifest.updateOrderbooks(markeProductGroup);
